...
**Versión actual : 1.4.6** (16 de mayo de 2026)
**Motor del juego : Unreal Engine 5.3.2 (IoStore)**

> 🆕 **v1.4.6** : recuperación de los 62 objetivos de tareas del Accionista que se habían quedado en inglés desde v1.0 debido al crash de recursión infinita. Nuevo patcher personalizado `BPOffsetPatcher` que resuelve dos problemas acumulados que ninguna herramienta existente trataba : (1) los offsets internos de los statements desplazados al final del bytecode y (2) los `EX_IntConst` de entry-point hardcodeados en los 47 callers internos del Blueprint. En el juego : `Ver la puesta de sol`, `Quedarse a bordo hasta las 21h`, `Evitar el desierto entre las 4h y las 18h`, `Recoge algo de miel: 0/3`, `Visitar el punto de foto del gran árbol`, etc. Reformulación gramatical de los fragmentos de carga. Quedan 2 strings `AM`/`PM` en inglés (bug menor de duplicados, no bloqueante).
...
- **Instalador Windows** (`ABR-es_v1.4.6.zip`, ~30-100 MB) : instalador PowerShell que detecta Steam automáticamente, ~3-5 min
- **Drop-in prepatched** (`ABR-es_v1.4.6_prepatched.zip`, ~1,9 GB) : reemplazo directo de los archivos de container, cualquier OS (Windows / Linux / Steam Deck / macOS), sin instalador

### Pasos (drop-in prepatched)

1. Descarga `ABR-es_v1.4.6_prepatched.zip` (ver [Releases](../../releases))
...
| Coexistencia FR/DE/ES | Solo un container `.ucas` activo a la vez - para cambiar de idioma, desinstalar uno (verificación de integridad Steam) e instalar el otro |
...
- **Algunas palabras permanecen en inglés en el QuestBoard y el ticket de misión** : `Lock` en el botón de candado en la parte superior del tablero, `DESTINATION:` en el ticket de misión lateral. Son identificadores internos UMG (los sub-componentes gráficos del widget) que causaban un crash si se traducían. Limitación conocida en v1.4.6, a corregir en una futura versión vía un enfoque alternativo.
