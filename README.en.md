...
> ⚠️ This repository ships **French, German, Spanish, and Japanese** translations. Pick the one you want (or none): the mod replaces the game's English UI, dialogue and notifications with the target language. The English-language audience can use this overview, but the in-game text becomes FR, DE, ES or JP depending on which package you install.
...
5. Launch the game through Steam normally. Menus should be in FR, DE, ES or JP depending on your chosen package.
...
Only one `.ucas` container can be active at a time. To switch between FR, DE, ES, and JP :
...
- **A few words stay in English on the QuestBoard and quest ticket** : `Lock` on the lock button above the quest tray, `DESTINATION:` on the side quest ticket. These are internal UMG identifiers (the widget sub-components) that caused a crash when translated. Known limitation as of v1.4.7, to be addressed in a future release via an alternative patching approach.
