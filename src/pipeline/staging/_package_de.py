"""
Pipeline de packaging DE.

Produit dans staging/legacy_patched_DE/ :
- BP patchés (KissE Replacement avec strings_BP.json)
...
Tous les vanilla viennent de staging/_release_legacy_vanilla_full/ (déjà extrait pour FR).
"""
...
WORKDIR = os.path.join(ROOT, 'staging', 'de_workdir')
...
os.makedirs(WORKDIR)
os.makedirs(PATCHED_DE, exist_ok=True)
...
def copy_vanilla_to_workdir():
    ...
    # Vanilla vient de _release_legacy_vanilla_full/ABumpyRide/Content/...
    ...
    bp_count = 0
    map_count = 0
    # On copie tout (BPs et maps) dans LEGACY_BP_DIR ; KissE prendra les .uasset
    # et on triera les .umap dans une seconde passe.
    for root, dirs, files in os.walk(src_root):
        for f in files:
            src = os.path.join(root, f)
            rel = os.path.relpath(src, src_root)
            # On veut que le chemin soit ABumpyRide/Content/<rel>
...
def kisse_replace_bp():
    """Run KissE Replacement on .uasset BPs using translations/de/strings_BP.json.
    ...
    run `python staging/_patch_bpop.py de` to translate them via BPOP (and TEST the
    ...
    with open(os.path.join(TRANS_DE, 'strings_BP.json'), 'r', encoding='utf-8-sig') as f:
        bp_data = json.load(f)
    ...
    print(f'  Excluded {excluded} entries (SpecialPassenger + QuestTicket, BPOP-only - run staging/_patch_bpop.py de after)')
...
def kisse_replace_maps():
    ...
    # Repère tous les .umap dans LEGACY_BP_DIR (ils y sont déjà via copy)
    umaps = []
    for root, dirs, files in os.walk(LEGACY_BP_DIR):
        for f in files:
            if f.endswith('.umap'):
    ...
    # Rename .umap → .uasset (et .umap.uexp si existe → .uexp)
    renamed = []
    for p in umaps:
        new = p.replace('.umap', '.uasset')
        os.rename(p, new)
        renamed.append((p, new))
        # le pair .uexp existe déjà tel quel — pas de rename pour lui

    # Adapter le JSON DE pour les maps (FileName: .umap → .uasset)
    maps_json_src = os.path.join(TRANS_DE, 'strings_maps.json')
    ...
    # Lance KissE sur le folder (il va parcourir les .uasset y compris les renamed)
    result = run([
        KISSE,
        json_in,
        LEGACY_BP_DIR,
...
def datatable_inject_enum(asset_path_rel, json_name):
    ...
    if os.path.exists(out) and os.path.exists(src_uexp):
        # datatable_text_patcher écrit le .uasset, mais il faut copier le .uexp depuis le src
        # (selon le code, l'outil copie déjà le uexp si nécessaire). Vérifions.
...
def inject_tutorial_table():
    ...
    if not os.path.exists(src):
        ...
        return
    strings_bp = os.path.join(TRANS_DE, 'strings_BP.json')
    ...
    if not tutorial_entries:
        ...
        return
    json_in = os.path.join(WORKDIR, '_tutorial_table_de_trad.json')
    with open(json_in, 'w', encoding='utf-8') as f:
...
def inject_skinbuttontable():
    ...
    json_in = os.path.join(TRANS_DE, 'skinbuttontable.json')
...
def consolidate():
    """Consolide tous les patches dans staging/legacy_patched_DE/.

    Stratégie : pour chaque fichier patché, comparer avec vanilla. S'il diffère, le copier
    dans legacy_patched_DE/. Sinon ignorer (= pas de modification effective).
    """
    print('\n=== Step 6 : consolidation -> staging/legacy_patched_DE/ ===')

    # Reset
    if os.path.exists(PATCHED_DE):
        # Sauvegarder _CONSOLIDATION_INFO.json précédent si présent
        old_info = os.path.join(PATCHED_DE, '_CONSOLIDATION_INFO.json')
        if os.path.exists(old_info):
            os.remove(old_info)
        # Vider le contenu (mais garder le dossier)
        for item in os.listdir(PATCHED_DE):
            p = os.path.join(PATCHED_DE, item)
            if os.path.isdir(p):
                shutil.rmtree(p)
            else:
                os.remove(p)
    else:
        os.makedirs(PATCHED_DE)

    # Sources à consolider :
    # 1. LEGACY_BP_DIR — tout ce qui diffère du vanilla
    # 2. ENUM_OUT_DIR — tous les enums patchés
    # 3. DT_OUT_DIR — SkinButtonTable
        ...
        for root, dirs, files in os.walk(src_root):
            for f in files:
                ...
                if file_differs(src, vanilla_eq):
                    dst = os.path.join(PATCHED_DE, rel)
                    os.makedirs(os.path.dirname(dst), exist_ok=True)
                    shutil.copy2(src, dst)
    ...
    info_path = os.path.join(PATCHED_DE, '_CONSOLIDATION_INFO.json')
